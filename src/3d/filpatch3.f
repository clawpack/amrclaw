...
c
c  only call physbd for coarse patches (where physbd sets the whole
c  patch. otherwise, orig. caller of filpatch3 will take care of this
c
	if ((nrowp .eq. mitot .and. ncolp .eq. mjtot)
     &                        .and. nfilp .eq. mktot ) then
c	   call physbd(valbig,aux,mitot,mjtot,mktot,nvar,naux,
c     1                 hxf,hyf,hzf,level,time,
c     2	               xlp,xrp,yfp,yrp,zbp,ztp,
...
c the boundary conditions. physbd will be called later, after all 4
...
900        format("starting at row: ",i4," cols ",i4," fil ",i4)
	   stop
        endif
...
c 2/28/02 : New call to
     ...
     4                 xperdom,yperdom,zperdom)
	   ...
	   write(*,900) nrowst,ncolst,nfilst
900        format("start at row: ",i4," col ",i4," fil ",i4)
	   stop
        endif
