...
c     # need to allocate for dynamic memory:
     ...
     1       ibuff,mstart,ndfree,lfine,iorder,mxnold,
 ...
 85             if (mptr .eq. 0) go to 94
     ...
     .                  (node(ffluxptr,mptr),2*nvar*lenbc+naux*lenbc)
                   ...
                   if (lev .gt. mxnest) then ! if level going away take away first storage
                      call reclam(node(store1,mptr),
     .                            mitot*mjtot*mktot*nvar)
                      node(store1,mptr) = 0
                      if (naux .gt. 0) then ! and aux arrays
                       call reclam(node(storeaux,mptr),
     .                             mitot*mjtot*mktot*naux)
